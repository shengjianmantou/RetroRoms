...
struct ScanProgress: Decodable { let filesScanned: Int; let totalFiles: Int }
  ...
  var body: some View {
    NavigationSplitView {
      VStack(alignment: .leading, spacing: 14) {
        ...
        Button("Add DAT files…") { model.chooseDatFiles() }
        Text(model.datPaths.isEmpty ? "Optional: add DATs for verified CRC/SHA matches" : "\(model.datPaths.count) DAT file\(model.datPaths.count == 1 ? "" : "s") selected").font(.caption).foregroundStyle(.secondary)
        Button(model.isScanning ? "Loading ROMs…" : "Load ROMs") { Task { await model.scan() } }.disabled(model.sourcePaths.isEmpty || model.isScanning)
        if model.isScanning { if model.totalFiles > 0 { ProgressView(value: Double(model.filesScanned), total: Double(model.totalFiles)) } else { ProgressView() }; Text(model.totalFiles > 0 ? "\(model.filesScanned) / \(model.totalFiles) files scanned" : "Finding files…").font(.caption).foregroundStyle(.secondary) }
...
@MainActor final class LibraryModel: ObservableObject {
  @Published var sourcePaths: [String] = []; @Published var datPaths: [String] = []; @Published var processedPath = ""; @Published var items: [CatalogItem] = []
  ...
  @Published var isScanning = false; @Published var isLoading = false; @Published var filesScanned = 0; @Published var totalFiles = 0
  var catalogURL: URL?; private var server: Process?; private var workingPath: String?; private let port = Int(ProcessInfo.processInfo.environment["RETROROMS_PORT"] ?? "") ?? Int.random(in: 20_000...60_000)
  func chooseSources() { let panel = NSOpenPanel(); panel.canChooseDirectories = true; panel.canChooseFiles = false; panel.allowsMultipleSelection = true; if panel.runModal() == .OK { sourcePaths = panel.urls.map(\.path); processedPath = ""; workingPath = nil; status = "Folders selected. Click Load ROMs to scan them." } }
  func chooseDatFiles() { let panel = NSOpenPanel(); panel.canChooseDirectories = false; panel.canChooseFiles = true; panel.allowsMultipleSelection = true; panel.message = "Choose DAT files for verified ROM identification"; if panel.runModal() == .OK { datPaths = panel.urls.map(\.path); status = "\(datPaths.count) DAT file\(datPaths.count == 1 ? "" : "s") selected. Click Load ROMs to apply them." } }
  ...
  func scan() async { guard let root = runtimeRoot() else { status = "Could not locate the RetroRoms runtime."; return }; guard let node = nodeExecutable() else { status = "Node.js 22 or newer was not found. Install Node or set RETROROMS_NODE."; return }; isScanning = true; filesScanned = 0; totalFiles = 0; defer { isScanning = false }; let catalogRoot = processedPath.isEmpty ? workingCatalogPath() : processedPath; let progressPath = URL(fileURLWithPath: catalogRoot).appendingPathComponent(".rom-curator/scan-progress.json"); status = "Scanning source folders safely…"; let args = [root.appendingPathComponent("apps/cli/src/main.ts").path] + sourcePaths.flatMap { ["--source", $0] } + datPaths.flatMap { ["--dat", $0] } + ["--processed", catalogRoot, "--progress-file", progressPath.path]; let poller = Task { [weak self] in while !Task.isCancelled { if let data = try? Data(contentsOf: progressPath), let progress = try? JSONDecoder().decode(ScanProgress.self, from: data) { self?.filesScanned = progress.filesScanned; self?.totalFiles = progress.totalFiles; self?.status = "Scanning \(progress.filesScanned) of \(progress.totalFiles) files…" }; try? await Task.sleep(for: .milliseconds(250)) } }; defer { poller.cancel() }; do { _ = try await run(node, args); catalogURL = URL(fileURLWithPath: catalogRoot).appendingPathComponent(".rom-curator/catalog.sqlite"); try await startServer(root, node: node); await reload(); status = "Scan complete: \(items.count) games. Select games, then choose Scrape artwork." } catch { status = "Scan failed: \(error.localizedDescription)" } }
  ...
  private func startServer(_ root: URL, node: String) async throws { server?.terminate(); let task = Process(); task.executableURL = URL(fileURLWithPath: node); task.arguments = [root.appendingPathComponent("packages/ui/src/server.ts").path, "--library", catalogURL!.path, "--port", "\(port)"]; try task.run(); server = task; try await Task.sleep(for: .milliseconds(350)) }
  private func workingCatalogPath() -> String { if let workingPath { return workingPath }; let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appendingPathComponent("RetroRoms/sessions/\(UUID().uuidString)", isDirectory: true); try? FileManager.default.createDirectory(at: base, withIntermediateDirectories: true); workingPath = base.path; return base.path }
